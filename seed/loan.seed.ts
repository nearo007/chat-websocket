...
const loanConfigs: LoanConfig[] = [
    {
        ...
        itemName: "Impressora 3D Prusa i3",
        loanQuantity: 1,
        loanDaysAgo: 2,
        dueDaysAfterLoan: 7,
        returned: false,
    ...
    {
        clientEmail: "carlos.pereira@email.com",
...
export async function seed(database: SeedDatabase) {
    const now = new Date();
    const admin = await database.user.findUnique({ where: { email: "admin@fablab.pt" } });
    if (!admin) throw new Error("Administrador do seed não encontrado.");

    for (const cfg of loanConfigs) {
        const client = await database.client.findUnique({ where: { email: cfg.clientEmail } });
        if (!client) throw new Error(`Cliente não encontrado: ${cfg.clientEmail}`);

        const item = await database.item.findFirst({ where: { name: cfg.itemName } });
        ...
        await database.loan.create({
            data: {
                ...
                returnedById: cfg.returned ? admin.id : null,
            },
        });
